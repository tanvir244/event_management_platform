...
export const POST = async (request) => {
    try {
        ...
        if (exist) {
            ...
            return new NextResponse(JSON.stringify({ message: "Event already exists" }), { status: 304 });
        }
        ...
        return new NextResponse(JSON.stringify({ message: "New event booked successfully" }), { status: 200 });

    } catch (error) {
        console.error('Error while booking event:', error);
        // Return an error response if something goes wrong
        return new NextResponse(JSON.stringify({ message: "An error occurred while booking the event" }), { status: 500 });
    }
};
